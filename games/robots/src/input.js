export function createControls({ send, onAction, enabled }) {
  ...
  const keydown = e => {
    ...
    if (e.repeat) return;
    keys.add(e.code); input(bindings[e.code] || null);
  ...
  document.querySelectorAll('[data-action]').forEach(button => {
    button.addEventListener('pointerdown', e => {
      ...
      input(button.dataset.action === 'block' ? null : button.dataset.action);
    });
    const release = e => { actionPointers.delete(e.pointerId); button.classList.remove('pressed'); input(); };
  ...
  const interval = setInterval(() => input(), 1000 / 30);
