...
import { buildV5Case } from './combat-v5-cases.js';
...
function record(type) {
  if(type==='finish')return buildV5Case('finish',facing);
  const room=new CombatRoom({id:'GLOW',random:()=>.8}),a=room.addPlayer('ПРОБОЙ'),b=room.addPlayer('ОПОРА');room.ready(a.id);room.ready(b.id);
  for(let i=0;i<181;i++)room.step(1/60);a.x=-1.25*facing;b.x=1.25*facing;a.facing=facing;b.facing=-facing;
  if(type==='ko')b.hp=5;
  const snapshots=[],events=[],seen=new Set();
  for(let frame=0;frame<240;frame++){
    if(frame===12&&type!=='lamps')room.input(a.id,{seq:a.lastSeq+1,move:0,block:false,crouch:false,action:'heavy'});
    room.step(1/60);const state=room.snapshot();for(const event of state.events)if(!seen.has(event.id)){seen.add(event.id);events.push({...event,frame});}snapshots.push(state);
...
const beat=()=>data.events.find(e=>e.type===($('case').value==='finish'?'destruction':'hit'))?.frame??0;
function choose(){data=record($('case').value);events=new Map();for(const e of data.events){if(!events.has(e.frame))events.set(e.frame,[]);events.get(e.frame).push(e);}$('frame').max=data.snapshots.length-1;reset();seek(beat()+($('case').value==='ko'?10:2));}
...
$('frame').oninput=()=>seek(Number($('frame').value));$('contact').onclick=()=>seek(beat()+2);$('arc').onclick=()=>seek(beat()+32);$('late').onclick=()=>seek(beat()+100);$('play').onclick=()=>playing=!playing;
...
function render(now){requestAnimationFrame(render);const dt=Math.min(.05,(now-last)/1000);last=now;if(playing){accumulator+=dt;while(accumulator>=1/60){accumulator-=1/60;at++;if(at>=data.snapshots.length){at=0;reset();}simulate(at);}}
  const lamps=$('case').value==='lamps';camera.position.set(close?(lamps?-5.8:.4):0,close?(lamps?4.1:2.9):3.8,close?6:10.7);camera.lookAt(close?(lamps?-5.8:.4):0,close?(lamps?4.7:1.2):1.5,lamps?-2.5:0);
