...
test('HUD consumes round, hit, grab and pause snapshots without breaking the receive-state loop', t => {
  ...
  assert.equal(buttons.filter(button => button.classList.contains('action-cued')).length, 0, 'idle instructions do not pretend to be an opportunity');
  ...
  assert.equal(button('dash').dataset.cue, 'СБРОСЬ · 50 ⚡');
  assert.equal(buttons.filter(button => button.classList.contains('action-cued')).length, 1);
  ui.update(state, 'p1');
  assert.equal(button('dash').dataset.cue, 'СБРОСЬ · 50 ⚡', 'repeated snapshot leaves a stable cue');
  ...
  assert.equal(button('light').dataset.cue, 'ВЫРВИСЬ!');
  assert.equal(button('dash').dataset.cue, '');
  state.phase = 'paused'; ui.update(state, 'p1');
  assert.equal(byId('escape-prompt').hidden, true);
  assert.equal(buttons.filter(button => button.classList.contains('action-cued')).length, 0);
  ...
  assert.equal(button('heavy').dataset.cue, 'ДОБЕЙ!');
  assert.equal(button('ultimate').dataset.cue, '', 'only one of the two valid finisher controls is prompted');
  state.phase = 'paused'; ui.update(state, 'p1');
