...
import { buildHeavyAdvantageCase, HEAVY_ADVANTAGE_CASES } from '/scripts/heavy-advantage-cases.js';
...
let data, selected = 'series', at = 0, facing = 1, playing = false, last = performance.now(), accumulator = 0, contactIndex = 0, simulated = -1, robots = [];
function reset() {
...
for (const [key, title] of Object.entries(HEAVY_ADVANTAGE_CASES)) {
...
function show(frame) {
  ...
  $('readout').textContent = `${p.name}: ${p.action}/${p.variant || 'обычный'} · ${p.hp} HP\nAction: ${p.actionTime.toFixed(3)} / ${p.actionDuration.toFixed(3)}s · Y ${p.y.toFixed(3)}\nТОЛЬКО ЗАЩИТА: ${p.defenseOnly.toFixed(3)}s\nПопадания: ${data.contacts.map(e => `${e.variant}@${e.frame}`).join(' · ')}\nИстория rig 60 Hz · без hitstop / FX / тряски`;
}
function pause() { playing = false; $('play').textContent = 'Продолжить'; }
function contactFrame() {
  const contact = data.contacts[contactIndex];
  if (contact) return contact.frame;
  const attack = data.events.find(event => event.type === 'attack');
  return attack ? attack.frame + Math.ceil(attack.startup * 60) : 0;
}
function jump(offset) { pause(); show(contactFrame() + offset); }
function choose(key) {
  selected = key; data = buildHeavyAdvantageCase(key, facing); reset(); contactIndex = key === 'cross' ? 1 : 0; accumulator = 0;
...
$('head').onclick = () => {
  const duration = data.snapshots[contactFrame()].players[1].actionDuration || .4;
  jump(Math.round(duration * (duration < .35 ? .69 : .55) * 60));
};
$('settle').onclick = () => jump(Math.ceil((data.snapshots[contactFrame()].players[1].actionDuration || .4) * 60));
$('next').onclick = () => { contactIndex = data.contacts.length ? (contactIndex + 1) % data.contacts.length : 0; jump(1); };
