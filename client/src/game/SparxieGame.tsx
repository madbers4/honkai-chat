  ...
  return (
    <main className="staff-page">
      ...
      <div className="staff-rules">
        <h2>На площадке</h2>
        <p>
          Отправь гостя к макету бомбы в стиле CS:GO с QR. Переписка без
          таймера. На лёгком уровне — три минуты, на среднем — четыре, на
          сложном — пять. Отсчёт начинается при открытии первого сапёра и идёт
          только на активном поле. Между попытками и во время реплик таймер
          стоит. По окончании времени Искра один раз добавит минуту и попытку.
          Обновление страницы продолжит ту же партию. Каждая вкладка играет
          отдельно; закрытие вкладки завершает её сессию.
        </p>
        <p>
          После финала гость показывает экран и получает печать. Если время или
          попытки закончились, бомба в истории срабатывает — раунд проигран.
                ...
                <p>
                  {game.phase === "won"
                    ...
                    : game.lossReason === "time"
                      ? "Даже дополнительная минута прошла — бомба сработала. Искра уже придумывает заголовок."
