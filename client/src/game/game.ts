...
function expire(game: Game, now: number): Game {
  ...
  return queue(
    ...
    now,
    "Всё, даже подаренная минута закончилась! Чат, встречаем финальный спецэффект — ВЗРЫВ!",
...
function advance(current: Game, action: Action, now: number): Game {
  ...
  if (board.exploded !== null) {
    ...
    if (last)
      lines.push(
        "Бомба сработала. Раунд за мной! Съёмочная группа просит больше так не рекламировать газировку.",
      );
    return queue(
      {
        ...game,
