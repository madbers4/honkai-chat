...
export function createCombatUI() {
  ...
  function update(state, playerId, intent = {}) {
    ...
    setText(recipe, threat ? actionCue?.key === 'overload-jump' ? 'ПРЫГАЙ СЕЙЧАС! · ДЖОЙСТИК ВВЕРХ' : threat.remaining < .15 ? 'РАЗРЯД · ДЕРЖИ БЛОК' : 'ВСЯ АРЕНА · СБЕЙ ЩИТ ИМПУЛЬСОМ ИЛИ СЕРИЕЙ ИЗ 3 УДАРОВ' : defenseOnly ? 'БЛОК / ПРЫЖОК / НАЗАД · АТАКА ВОССТАНАВЛИВАЕТСЯ' : tip);
