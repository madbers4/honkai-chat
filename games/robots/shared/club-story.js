import { MAX_HP, WINS_TO_MATCH, ROUND_SECONDS, ATTACKS, ULTIMATE_HOLD_SECONDS, HEAVY_RULES, V5_ATTACKS } from './constants.js';
...
export const RULE_CARDS = Object.freeze([
  { id: 'thumbs', title: 'Два пальца — одна судьба', text: 'Поверните телефон горизонтально. Слева движение и прыжок, справа приёмы и защита.', readAloud: 'Телефоны поперёк, большие пальцы на позиции. Слева командуем ногами, справа принимаем решения, о которых потом расскажем механику.' },
  ...
  { id: 'series', title: 'Крепёж любит последовательность', text: 'Лёгкие удары складываются в серию. Тяжёлый удар требует замаха; продолжение открывается после попадания.', readAloud: 'Лёгкие удары нажимаем ритмично: получится серия. Тяжёлый сначала замахивается. Попал — можно продолжить крюком и прессом. Промахнулся — публика подождёт, пока вы соберётесь.' },
  ...
  { id: 'special', title: 'Электричество за свой счёт', text: `Импульс бьёт вперёд; спецприём в приседе ставит мину. Перегрузка стоит ${ATTACKS.ultimate.energy} энергии: удерживайте кнопку ${String(ULTIMATE_HOLD_SECONDS).replace('.', ',')} с.`, readAloud: `Импульс стреляет вперёд. Спецприём в приседе оставляет электрическую мину рядом с машиной. Для перегрузки накопите ${ATTACKS.ultimate.energy} энергии и удержите кнопку чуть дольше половины секунды. Очень торжественно, но противник ждать не обязан.` },
