...
test('dash cancels into an advancing, vulnerable claw strike and the follow-up window expires', () => {
  ...
  assert.equal(room.player('p2').hp, 89);
  assert.ok(room.damage(room.player('p2'), attacker, ATTACKS.light, 'light'));
  assert.equal(attacker.hp, 93, 'dash invulnerability ends at the attack cancel');
  const late = roomAt(4);
...
test('only a confirmed light opens launcher, and launcher supports an actual jump/light follow-up', () => {
  ...
  assert.equal(room.player('p2').hp, 83);
  ...
  input(room, 'p1', { action: 'light' });
  advance(room, 0.18);
  assert.ok(room.player('p2').hp < 83, 'air follow-up really connects');
...
test('air heavy dives and hits exactly once on real landing, exposing landing-relative recovery', () => {
  ...
  assert.equal(room.player('p2').hp, 82);
  assert.ok(player.landedTime > 0.1);
  assert.ok(Math.abs(player.actionDuration - player.landedTime - 0.42) < 1e-8);
...
test('crouch-special latches its variant, cannot be ducked, and a jump clears its low path', () => {
  ...
  assert.equal(wave.y, 0.3);
  assert.equal(wave.speed, 7.4);
  advance(crouched, 0.4, { p2: { crouch: true } });
  assert.equal(crouched.player('p2').hp, 86);
  ...
  assert.equal(jumped.player('p2').hp, 100);
});

test('held block protects against a ground wave after the perfect-parry window has passed', () => {
  ...
  assert.equal(room.player('p2').hp, 98);
  assert.equal(room.player('p2').guard, 71);
...
test('fresh directional block parries melee without damage, then awards exactly one stronger counter hit', () => {
  ...
  assert.equal(room.player('p1').hp, 88);
...
test('holding or rapidly toggling block cannot renew a parry, and rear attacks cannot be parried', () => {
  ...
  assert.equal(room.player('p2').hp, 93);
...
test('overload emits exactly three authoritative pulses and deals at most 44 unscaled damage', () => {
  const room = roomAt(4);
  room.player('p1').energy = 100;
  room.player('p1').counterWindow = 1.2;
  input(room, 'p1', { action: 'ultimate' }); advance(room, 0.69);
  assert.equal(room.player('p2').hp, 100);
  advance(room, 0.05);
  assert.equal(room.player('p2').hp, 88);
  advance(room, 0.31);
  assert.equal(room.player('p2').hp, 76);
  advance(room, 0.31);
...
test('an ultimate ignores parry and its telegraph allows an early retreat out of pulse range', () => {
  ...
  input(blocked, 'p1', { action: 'ultimate' }); advance(blocked, 0.62);
  input(blocked, 'p2', { block: true }); advance(blocked, 0.13, { p2: { block: true } });
  assert.equal(blocked.player('p2').hp, 98);
  assert.equal(blocked.player('p2').guard, 72);
  ...
  advance(retreat, 1.5, { p2: { move: 1 } });
  assert.equal(retreat.player('p2').hp, 100);
});
...
test('disconnect freezes reward windows and pulse progression; resume does not duplicate an already-fired pulse', () => {
  ...
  input(room, 'p1', { action: 'ultimate' }); advance(room, 0.8);
  assert.equal(room.player('p2').hp, 88);
  const remaining = room.player('p1').launchWindow;
  room.setConnected('p2', false); advance(room, 5);
  assert.equal(room.player('p1').launchWindow, remaining);
...
test('extended seeded combat keeps physics, resources, cooldowns and new windows bounded', () => {
  ...
  for (let frame = 0; frame < 60 * 120; frame++) {
    ...
    for (const player of room.players) {
      ...
      assert.ok(player.hp >= 0 && player.hp <= 100);
