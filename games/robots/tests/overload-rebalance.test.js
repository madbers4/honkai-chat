...
test('80-energy overload has an armored .95s windup and a lethal 220 damage sequence', () => {
  ...
  step(room, .93); assert.equal(b.hp, MAX_HP); assert.equal(a.action, 'ultimate');
  step(room, .04); assert.equal(b.hp, MAX_HP - 45); assert.equal(b.variant, 'overloadHit');
  step(room, .28); assert.equal(b.hp, MAX_HP - 100); assert.equal(b.variant, 'overloadHit');
  step(room, .32); assert.equal(b.hp, 0); assert.equal(room.phase, 'roundOver');
...
test('leaving marked range, stepping behind or interrupting windup all defeat the overload', () => {
  ...
  step(retreat, .12); input(retreat, 'p2', { action: 'dash', move: 1 }); step(retreat, 1.6, { p2: { move: 1 } });
...
test('a confirmed airborne overload cannot lose damage to old juggle scaling or early knockback', () => {
  ...
  step(room, .9); b.y = .6; b.vy = 1; b.airHits = 10;
  step(room, .1); assert.equal(b.hp, MAX_HP - 45); assert.equal(b.vx, 0); assert.ok(b.actionDuration >= .35);
  step(room, .6); assert.equal(b.hp, 0);
});

test('disconnect pauses the exact pulse sequence and resume never replays its first strike', () => {
  const room = fight(); room.beginAction(room.player('p1'), 'ultimate'); step(room, 1.03);
  ...
  assert.equal(room.player('p2').hp, 0); assert.equal(ATTACKS.ultimate.duration, 2.28);
});
